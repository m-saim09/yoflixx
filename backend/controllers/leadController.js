...
const getLeadList = asyncHandler(async (req, res) => {
  ...
  if (plan) {
    const planAliases = {
      "Level 1": ["Level 1", "Starter"],
      "Level 2": ["Level 2", "Growth"],
      "Level 3": ["Level 3", "Enterprise"],
      Starter: ["Starter", "Level 1"],
      Growth: ["Growth", "Level 2"],
      Enterprise: ["Enterprise", "Level 3"],
    };

    const values = planAliases[plan] || [plan];
    filter.selectedPlan = values.length > 1 ? { $in: values } : plan;
  }
...
const updateLead = asyncHandler(async (req, res) => {
  ...
  // allow updating read state explicitly
  if (typeof req.body.isRead !== "undefined") {
    updates.isRead = Boolean(req.body.isRead);
  }
