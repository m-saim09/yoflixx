...
const updateWebsiteSettings = asyncHandler(async (req, res) => {
  ...
  // deep merge helper to safely merge nested objects (arrays are replaced)
  ...
  const asObject = (val) => (val && typeof val.toObject === 'function' ? val.toObject() : val || {});

  settings.businessInfo = {
    ...asObject(settings.businessInfo),
    ...(req.body.businessInfo || {}),
  };
  settings.heroSection = {
    ...asObject(settings.heroSection),
    ...(req.body.heroSection || {}),
  };
  settings.aboutSection = {
    ...asObject(settings.aboutSection),
    ...(req.body.aboutSection || {}),
  };
  settings.contactInfo = {
    ...asObject(settings.contactInfo),
    ...(req.body.contactInfo || {}),
  };
  settings.socialLinks = {
    ...asObject(settings.socialLinks),
    ...(req.body.socialLinks || {}),
  };
  settings.branding = {
    ...asObject(settings.branding),
    ...(req.body.branding || {}),
  };
  settings.seoSettings = {
    ...asObject(settings.seoSettings),
    ...(req.body.seoSettings || {}),
    keywords: keywordsValue(req.body.seoSettings?.keywords ?? (asObject(settings.seoSettings).keywords || [])),
  };
  ...
  // Merge any nested contentSections safely so partial updates won't clobber existing data
  if (req.body.contentSections && typeof req.body.contentSections === 'object') {
    const prev = typeof settings.contentSections?.toObject === 'function' ? settings.contentSections.toObject() : settings.contentSections || {};
    settings.contentSections = deepMerge(prev, req.body.contentSections || {});
  }
